...
def main():
    ...
    tau1 = coef['TAU1'].values.astype(np.float64)
    tau2 = coef['TAU2'].values.astype(np.float64)
    theta = coef['THETA'].values.astype(np.float64)
    ...
    annot_snps, all_annot, annot_mat = load_annot(argmap['annot'],
    ...
    # create binary annotations based on continuous annotations
    bin_annot_names, bin_annot_mat = get_bin_annot_mat(annot_mat, all_annot,
        argmap['names'], argmap['nbins'])

    # get summary information for annotation matrix
    annot_nsnp, annot_std = get_annot_sumstat(bin_annot_mat)

    # get hsq and gcov for each annotation
    hsq1, hsq1_se, ps_hsq1 = get_sum_bin(annot_mat,tau1,ps_tau1,bin_annot_mat)
    hsq2, hsq2_se, ps_hsq2 = get_sum_bin(annot_mat,tau2,ps_tau2,bin_annot_mat)
    gcov, gcov_se, ps_gcov = get_sum_bin(annot_mat,theta,ps_theta,bin_annot_mat)
    ...
    gcor,gcor_se = get_gcor(hsq1, ps_hsq1, hsq2, ps_hsq2, gcov, ps_gcov, False)
...
def get_bin_annot_mat(annot_mat, all_annot, names, nbins):
    ...
    new_annot_mat[:,0] = np.ones(nsnp, dtype=np.float32)
    ...
    for j in xrange(len(names)):
        ...
        for i in range(cut.shape[0]-1):
            ...
            if i == 0: low -= 0.00001 
            high = cut[i+1]
            updt_idx = np.where((annot_vec>low) & (annot_vec<=high))[0]
            new_annot_mat[updt_idx, idx] = 1.0
            idx += 1
            annot_mean = np.mean(annot_vec[updt_idx])
            annot_median = np.median(annot_vec[updt_idx])
            nsnp = updt_idx.shape[0]
            logging.info('{} {}-th bin: interval ({}, {}], nsnp {},  '
                         'mean {}, median {}'.format(nm, (i+1), low, high,
                            nsnp, annot_mean, annot_median))
            
    return new_names, new_annot_mat

# load jack knife coefficient
def load_pseudo_coef(filename):
    ps_coef = np.loadtxt(filename, dtype=np.float64)
    return ps_coef
...
def get_command_line():
    ...
    parser.add_argument('--nbins', dest='nbins', type=int,
        required=False, help='Number of continuous annotation bins')

    parser.add_argument('--names', dest='names', type=str, nargs='+',
        required=False, help='A list of continuous annotations')
